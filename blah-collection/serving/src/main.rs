...
use repo::{CollectionQuery, CollectionRepo};
...
impl CollectionService {
    fn collections(&self, req: Request) -> FutureResponse {
        let repo = self.repo.clone();
        Box::new(Self::deserialize::<CollectionQuery>(req)
...
fn main() {
    ...
    let work = listener.incoming().for_each(move |(socket, addr)| {
        ...
        let service = CollectionService { repo: repo };
