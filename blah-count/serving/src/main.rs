...
use count::{CountRepo, CountQuery};
...
struct CountService {
    count_repo: CountRepo,
    sum_repo: SumRepo,
    most_viewed_repo: MostViewedRepo,
}

impl CountService {
    fn count(&self, req: Request) -> FutureResponse {
        let repo = self.count_repo.clone();
        Box::new(Self::deserialize::<CountQuery>(req)
            .and_then(move |result| match result {
                Ok(query @ CountQuery { group_by: None, .. }) => {
                    Box::new(repo.count(query).map(|resp| {
                        Self::new_response(resp)
                    })) as FutureResponse
                },
                Ok(query) => {
                    Box::new(repo.grouped(query).map(|resp| {
                        Self::new_response(resp)
                    })) as FutureResponse
                },
                Err(_) => {
    ...
    fn get_most_viewed(&self, req: Request) -> FutureResponse {
        let repo = self.most_viewed_repo.clone();
        Box::new(Self::deserialize::<MostViewedQuery>(req)
            .and_then(move |result| match result {
                Ok(query) => {
    ...
    fn sum(&self, req: Request) -> FutureResponse {
        let repo = self.sum_repo.clone();
        Box::new(Self::deserialize::<SumQuery>(req)
            .and_then(move |result| match result {
                Ok(query) => {
                    Box::new(repo.sum(query).map(|resp| {
                        Self::new_response(resp)
                    })) as FutureResponse
                },
                Err(_) => {
...
impl Service for CountService {
    ...
    fn call(&self, req: Self::Request) -> Self::Future {
        match (req.method(), req.path()) {
            ...
            (&Method::Get, "/count.js")             => Self::assets("count.js"),
            (&Method::Get, "/most-viewed.js")       => Self::assets("most-viewed.js"),
            (&Method::Get, "/sum.js")               => Self::assets("sum.js"),
            (&Method::Get, "/segmentation.js")      => Self::assets("segmentation.js"),
            (&Method::Get, "/segmentation.css")     => Self::assets("segmentation.css"),
...
fn main() {
    ...
    let work = listener.incoming().for_each(move |(socket, addr)| {
        let most_viewed_repo = MostViewedRepo {
            client: client.clone(),
            elastic_url: elastic_url.clone(),
        };

        let sum_repo = SumRepo {
            client: client.clone(),
            elastic_url: elastic_url.clone(),
        };

        let count_repo = CountRepo {
            client: client.clone(),
            elastic_url: elastic_url.clone(),
        };
        ...
        let service = CountService {
            ...
            sum_repo: sum_repo,
            most_viewed_repo: most_viewed_repo,
        };
