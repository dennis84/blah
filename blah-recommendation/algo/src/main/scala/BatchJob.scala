...
object BatchJob {
  def run(
    ...
    val eventsDf = spark.sql("SELECT collection, props as p FROM events")
      .withColumn("props", from_json($"p", RecommendationPropsSchema()))
      .drop("p")
      .toDF

    val df = spark.createDataFrame(eventsDf.rdd, RecommendationSchema())
    df.createOrReplaceTempView("events")
    
    val output = RecommendationAlgo.train(spark, args shift "path")
    output.writeToElastic("recommendation", "recommendation")
    ...
    output.toJSON.writeToKafka(props, x =>
      new ProducerRecord[String, String]("trainings", s"recommendation@$x"))
