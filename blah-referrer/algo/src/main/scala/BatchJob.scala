...
object BatchJob {
  def run(
    ...
    val eventsDf = spark.sql("SELECT collection, props as p FROM events")
      .withColumn("props", from_json($"p", ReferrerPropsSchema()))
      .drop("p")
      .toDF

    val df = spark.createDataFrame(eventsDf.rdd, ReferrerSchema())
    df.createOrReplaceTempView("events")
    
    val output = ReferrerAlgo.train(spark, args shift "path")
    output.writeToElastic("referrer", "referrer")
    ...
    output.toJSON.writeToKafka(props, x =>
      new ProducerRecord[String, String]("trainings", s"referrer@$x"))
