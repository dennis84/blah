...
use repo::{ReferrerQuery, ReferrerRepo};
...
struct ReferrerService {
    repo: ReferrerRepo,
}

impl ReferrerService {
    fn list(&self, req: Request) -> FutureResponse {
        let repo = self.repo.clone();
        Box::new(Self::deserialize::<ReferrerQuery>(req)
...
impl Service for ReferrerService {
    ...
    fn call(&self, req: Self::Request) -> Self::Future {
        match (req.method(), req.path()) {
            ...
            (&Method::Get, "/js/referrer.js") => Self::js(),
...
fn main() {
    ...
    let work = listener.incoming().for_each(move |(socket, addr)| {
        let repo = ReferrerRepo {
            client: client.clone(),
            elastic_url: elastic_url.clone(),
        };

        let service = ReferrerService { repo: repo };
